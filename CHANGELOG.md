...
- `npub serve` once again defaults to the configured `build_path` rather than a notes path. Falls back to `./dist` when no config is found; surfaces config errors when a config was explicitly requested.
- Override the build directory with a positional `[dir]` argument on both `npub build` and `npub serve` (e.g. `npub build ./public`). Replaces `build --out` and `serve --dir`.
...
- Drop `build --out` and `serve --dir`; use the positional `[dir]` instead.
